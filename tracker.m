function [bboxes, speed] = tracker(varargin)
    %% default hyper-params for HSSNet tracker. most of them are the same as CFNet
    p.join.method = 'corrfilt';
    p.net = 'net-epoch-37-HSSNet.mat';
    p.numScale = 3;
    p.scaleStep = 1.0375;
    ...
    for i = p.startFrame:nImgs
        ...
        if p.visualization
            if isempty(videoPlayer)
                ...
                im = gather(im)/255;
                im = insertShape(im, 'Rectangle', rectPosition, 'LineWidth', 4, 'Color', 'green');
                im = insertText(im,[10 10],i,'FontSize',18, 'BoxOpacity',0.4,'TextColor','yellow');                          
